"""
scheduler/nyse_calendar.py

Determines whether the NYSE (US stock market) is currently open.
Ported from the OLD/market_schedule.py bot — self-contained, no API calls.
...
def get_nyse_holidays(year: int) -> set[date]:
    ...
    NYSE holidays: New Year's Day, MLK Jr. Day, Presidents' Day, Good Friday,
    Memorial Day, Juneteenth (since 2022), Independence Day, Labor Day,
    Thanksgiving Day, Christmas Day. Each is observed on the nearest weekday
    when it falls on a weekend.
    """
    h: set[date] = set()

    h.add(_nearest_weekday(date(year, 1, 1)))                     # New Year's Day
    h.add(_nth_weekday(year, 1, 0, 3))                              # MLK Jr. Day
    ...
    if year >= 2022:
        h.add(_nearest_weekday(date(year, 6, 19)))                 # Juneteenth

    h.add(_nearest_weekday(date(year, 7, 4)))                       # Independence Day
    h.add(_nth_weekday(year, 9, 0, 1))                              # Labor Day
    h.add(_nth_weekday(year, 11, 3, 4))                             # Thanksgiving Day
    h.add(_nearest_weekday(date(year, 12, 25)))                     # Christmas Day
...
def get_nyse_early_closes(year: int) -> set[date]:
    ...
    jul4 = date(year, 7, 4)
    if jul4.weekday() == 5:
        pass  # July 4 is Saturday -> holiday observed July 3 (full holiday)
    elif jul4.weekday() == 6:
        ec.add(date(year, 7, 3))  # July 4 is Sunday -> July 3 still early close
    else:
        d = date(year, 7, 3)
        if d.weekday() < 5:
            ec.add(d)
...
def seconds_until_market_open(now: datetime | None = None) -> float:
    ...
    return 86_400.0  # fallback — should never be reached in practice
...
def market_status_str(now: datetime | None = None) -> str:
    ...
    if today in holidays:
        return f"closed - NYSE holiday ({today.strftime('%A, %B')} {today.day})"
